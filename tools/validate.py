    ...
    for u in UNITS:
        ...
        for lid in u["syllabus"]["lexis"]:
            ...
            for f in l["spelling"]["inflections"]:
                forms.add(f.lower()); forms.update(f.lower().split())
...
def _target_words(gids):
    out = set()
    for gid in gids or []:
        g = GRAM.get(gid)
        if g:
            out |= {w.lower() for w in re.findall(r"[A-Za-z']+",
                                                  g.get("form","") + " " + g.get("label",""))}
    return out
...
def gate_vocab_grammar():
    ...
    for it in ITEMS:
        ...
        forms = cache[idx] | _target_words(it["targets"].get("grammar"))
        texts = []
        p, a = it["prompt"], it["answer"]
        for k in ("text",):
            if p.get(k) and it["type_id"] != "error_correct": texts.append(p[k])
        for k in ("options", "bank", "key", "accepted"):
            v = p.get(k) or a.get(k) or []
            texts += [x for x in v if isinstance(x, str) and x.lower() not in UI_CHROME]
        ...
        for t in texts:
            for w in WORD_RE.findall(t):
                lw = w.lower()
                if lw in forms or lw in NAMES or lw in METALANGUAGE or lw in STRUCTURAL: continue
                if lw.endswith("'s") and lw[:-2] in forms: continue
    ...
    for tid, s in STORIES.items():
        ...
        for line in s["body"]:
            for w in WORD_RE.findall(line["text"]):
                lw = w.lower()
                if lw in forms or lw in NAMES or lw in METALANGUAGE or lw in STRUCTURAL: continue
                # possessive 's is generated by G-011; it is known once its base noun is
                if lw.endswith("'s") and (lw[:-2] in forms or lw[:-2] in NAMES): continue
